...
function camada(look, slot) {
  const item = look[slot];
  const peca = item && pecaPorId(item.id);
  if (!peca || peca.slot !== slot) return '';
  const [x, y] = ANCORAS[slot];
  return `<g transform="translate(${x} ${y})" data-peca="${peca.id}">${peca.desenho(corPorNome(item.cor))}</g>`;
}

/**
 * @param {object} look `{ slot: { id, cor } }`, com slots de roupas.js e cor pelo nome de CORES.
 *                      Ids desconhecidos são ignorados, então um look antigo nunca quebra o desenho.
 */
export function princesa(look = {}) {
  const corpo = look.corpo && pecaPorId(look.corpo.id);
  const base = !(corpo && corpo.semBase);
